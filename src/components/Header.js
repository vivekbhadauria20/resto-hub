    ...
    <header className="fixed z-50 w-screen p-3 px-4 md:p-6 md:px-16 bg-primary">
      ...
      <div className="hidden md:flex w-full h-full items-center justify-between">
        <Link to={"/"} className="flex items-center gap-2">
          ...
          <p className="text-headingColor text-xl font-bold">RespoHub</p>
        </Link>
        <div className="flex items-center gap-8">
          ...
          <div
            className="realitive flex items-center justify-center"
            onClick={showCart}
          >
            <MdShoppingBasket className="text-textColor text-2xl cursor-pointer" />
            {cartItems && cartItems.length > 0 && (
              <div className="absolute top-6 w-5 h-5 rounded-full bg-cartNumBg flex items-center justify-center">
          ...
          <div className="relative">
            <motion.img
              ...
              className="w-10 min-w-[35px] h-10 min-h-[35px] drop-shadow-xl cursor-pointer rounded-full"
              alt="userprofile"
              onClick={login}
            />
            {isMenu && (
      ...
      <div className="flex items-center justify-between md:hidden w-full h-full">
        ...
        >
          <MdShoppingBasket className="text-textColor text-2xl cursor-pointer" />
          {cartItems && cartItems.length > 0 && (
            <div className="absolute top-3 w-5 h-5 rounded-full bg-cartNumBg flex items-center justify-center">
              <p className="text-xs text-white font-semibold">
                {cartItems.length}
              </p>
            </div>
        ...
        <div className="relative">
          ...
          {isMenu && (
            <div
              ...
              className="w-40 bg-gray-50 shadow-xl rounded-lg flex flex-col absolute top-14 right-0"
            >
              {user && user.email === "shreyansh1029@gmail.com" && (
                <Link to={"/createItem"}>
                  <p
                    className="px-4 py-2 flex items-center gap-3 cursor-pointer hover:bg-slate-100 transition-all duration-100 ease-in-out text-textColor text-base"
                    onClick={() => setIsMenu(false)}
