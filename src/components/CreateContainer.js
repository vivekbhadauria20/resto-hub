...
const CreateContainer = () => {
    ...
    uploadTask.on(
      ...
      () => {
        getDownloadURL(uploadTask.snapshot.ref).then((downloadUrl) => {
          ...
          setIsLoading(false);
          setFields(true);
          setMsg("Imgae uploaded succesfully");
  ...
  const deleteImage = () => {
    ...
    deleteObject(deleteRef).then(() => {
      ...
      setIsLoading(false);
      setFields(true);
      setMsg("Imgae deleted succesfully");
      setAlertStatus("success");
      setTimeout(() => {
        setFields(false);
      }, 4000);
    ...
    <div className="w-full min-h-screen flex items-center justify-center">
      <div className="w-[90%] md:w-[75%] border border-gray-200 rounded-lg p-4 flex flex-col items-center justify-center gap-4">
        ...
        <div className="flex items-center w-full">
          <button
            type="button"
            className="ml-0 md:ml-auto w-full md:w-auto border-none outline-none bg-emerald-500 px-12 py-2 rounded-lg tezt-lg text-white font-semibold"
            onClick={saveDetails}
          >
            Save
          </button>
